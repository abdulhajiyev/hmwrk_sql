...
ALTER TABLE Posts
ADD CHECK([Name]='assistant professor' OR [Name]='assistant' OR [Name]='professor' OR [Name]='teacher')
...
--16.1. All job titles.
CREATE VIEW [All job titles] AS SELECT [Name] AS [Job Title] FROM Posts;

--16.2. All the names of teachers.
CREATE VIEW [All the names of teachers] AS SELECT [Name] AS [Teachers] FROM Teachers;

--16.3. The identifier, the name of the teacher, his position, the general s / n (sort by s \ n).
CREATE VIEW [View_16.3] AS SELECT Teachers.Id AS [identifier], Teachers.[Name] AS [Teacher],Posts.[Name] AS [Position], Salary AS [Salary] FROM Teachers JOIN Posts ON Posts.Id=Teachers.IdPost;

--16.4. Identification number, surname, telephone number (only those who have a phone number)
CREATE VIEW [View_16.4] AS SELECT Teachers.Id AS [identifier], Teachers.[Name] AS [Teacher],Teachers.Phone FROM Teachers JOIN Posts ON Posts.Id=Teachers.IdPost WHERE Teachers.Phone IS NOT NULL;

--16.5. Surname, position, date of admission in the format [dd/mm/yy]
CREATE VIEW [View_16.5] AS SELECT Teachers.[Name] AS [Teacher],Posts.[Name] AS [Position], FORMAT(Teachers.HireDate,'dd-MM-yy') AS [Hire Date] FROM Teachers JOIN Posts ON Posts.Id=Teachers.IdPost;

--16.6. Surname, position, date of receipt in the format [dd month_text yyyy].
CREATE VIEW [View_16.6] AS SELECT Teachers.[Name] AS [Teacher], Posts.[Name]AS [Position], FORMAT(Teachers.HireDate,'dd-MMMM-yyyy') AS [Hire Date] FROM Teachers JOIN Posts ON Posts.Id=Teachers.IdPost;